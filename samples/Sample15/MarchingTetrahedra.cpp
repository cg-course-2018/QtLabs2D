...
bool IsoVertex::operator==(const IsoVertex &other)
{
    return id == other.id;
}
...
bool IsoEdge::operator==(const IsoEdge &other)
{
...
{
    const float distance = distance(m_position, position);
...
bool IsoCube::isIntersected(float threshold) const
{
    ...
    for (unsigned i = 0; i < m_vertexes.size();)
    {
...
void IsoSurface::setSources(std::vector<IIsoSourcePtr> &&sources)
{
...
void IsoSurface::calculateNormalsForPoints()
{
    ...
    {
        IsoPoint &p1 = m_points[i];
        IsoPoint &p2 = m_points[i + 1];
        IsoPoint &p3 = m_points[i + 2];
...
vec3 IsoSurface::getPointAtThresholdBetween(const vec3 &lowPoint, const vec3 &highPoint) const
{
    ...
    const float delta = getFieldStrengthAtPosition(midpoint) - m_threshold;
