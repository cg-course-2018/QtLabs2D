...
namespace
{
	...
	void DoWithBindedArrays(const std::vector<SVertexP3NT2> &vertices, T && callback)
	{
		...
		glEnableClientState(GL_VERTEX_ARRAY); //СТАРЫЕ МЕТОДЫ РИСОВАНИЯ!!!!
		glEnableClientState(GL_NORMAL_ARRAY);//СТАРЫЕ МЕТОДЫ РИСОВАНИЯ!!!!
		glEnableClientState(GL_TEXTURE_COORD_ARRAY);//СТАРЫЕ МЕТОДЫ РИСОВАНИЯ!!!!
