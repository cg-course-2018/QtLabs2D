...
MeshDataP3C3N3 IsoSurface::createGeometry(const Material &material)
{
	m_points.clear();
	...
	std::vector<VertexP3C3N3> vertexes(m_points.size());
	std::transform(m_points.begin(), m_points.end(), vertexes.begin(), [&](const IsoPoint &p) {
		...
		return VertexP3C3N3{ p.position, whiteColor, averageNormal };
	});
